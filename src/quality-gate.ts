...
export function validateEntity(entity: PlaceEntity, ctx: QualityContext): QualityResult {
  ...
  const SUPERLATIVES = /بهترین|زیباترین|منحصربهفرد|منحصر به فرد|اولین|قدیمیترین|جادویی|بینظیر|شگفتانگیز|فراموشنشدنی|ایدهآل|فوقالعاده|انقلابی|بهترین تجربه زندگی|بکر|بیهمتا|مقصد ایدهآل|زیباترین روستا|بهترین مقصد|بهترین روستا/g;
  const TECH_NOISE = /پلتفرم جامع|راهکار نوین|هوش مصنوعی پیشرفته|الگوریتم هوشمند|خدمات یکپارچه|هوش مصنوعی|فناوری پیشرفته|فناوری سنتی|سیستم هوشمند|سیستم مدیریت|مدیریت گردشگری|تکنولوژی|دیجیتال|پلتفرم|نرمافزار/g;
  const CLICHES = /تجربه منحصربهفرد|سفر رؤیایی|سبک زندگی پویا|جامعه کاربری گسترده|امنیت کاملاً تضمینشده|فراهم میکند|ارائه میدهد|ایجاد کرده است|به سطحی بالاتر|تجربه زندگی|مقصدی ایدهآل|مکان جادویی|همیشه تجربهای|در دنیای پرشتاب امروز|در دنیای امروز|آیا میدانستید|تصور کنید|همین حالا شروع کنید|فرصت را از دست ندهید/g;
